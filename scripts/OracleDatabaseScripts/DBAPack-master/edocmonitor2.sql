...
set termout off
select to_char(trunc(sysdate-1)+(14/24+30/1440), 'DD/MM/YYYY HH24:MI:SS') start_date from dual;
...
prompt
prompt ############################################################################################
prompt Sentenças EDOC que demoraram mais de &nlimite. segundos, a partir de &START_DATE.
rem prompt NOTA: nx_prepare_user_read_acls(:1) excluída da listagem
prompt ##################################################
...
FROM (
  ...
  and to_date(x1.sql_exec_start, 'MM/DD/YYYY HH24:MI:SS') between to_date( '&START_DATE.', 'DD/MM/YYYY HH24:MI:SS' ) and (to_date( '&START_DATE.', 'DD/MM/YYYY HH24:MI:SS' )+6/24)
  order by 5 desc 
