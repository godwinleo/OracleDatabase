...
rem exec dbms_output.put_line('sqlid: ' || :sqlid)

with qr as 
(
   select /*+ no_merge(v) materialize */ level id, to_char(substr(sf,(level-1)*2000+1,2000)) sql_text
            from 
            (
               select sql_fulltext sf from v$sqlarea where sql_id=:sqlid
               union all
...
(
    ...
    from 
    (   
      select /*+no_merge*/ 0 PLAN_HASH_VALUE, 11 seq, ID, sql_text
      from qr
      where instr(:showOptions,'T')>0
      order by id
    )
...
rem exec dbms_output.put_line('show: ' || :showOptions)
