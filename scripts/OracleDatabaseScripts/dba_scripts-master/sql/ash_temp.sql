...
select *
from (
	select  SAMPLE_ID, to_char(min(ash.sample_time),'YYYY-MON-DD HH24:MI') start_time, to_char(max(ash.sample_time),'YYYY-MON-DD HH24:MI') end_time
	...
	group by SAMPLE_ID,session_id, session_serial#,ash.user_id,u.username, ash.sql_id, ash.sql_plan_hash_value
