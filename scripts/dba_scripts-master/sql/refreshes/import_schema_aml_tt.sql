...
declare
    ...
    v_row  PLS_INTEGER;
begin
	v_job_name := 'IMP_TT\@'||v_network_link;
	...
	*/						
	...
	DBMS_DATAPUMP.metadata_filter(	Handle => v_dp_job,
									Name => 'SCHEMA_EXPR',
									value => 'IN (''EXT$TTMN_AML'',''EXT$TT_MNT'')'
								);
	...
	*/
	...
	DBMS_DATAPUMP.DATA_FILTER (v_dp_job, 'SUBQUERY', 'WHERE to_date(V_MIS_DATE,''YYYYMMDD'') >= add_months(trunc(sysdate,''MM''),0)', 'AML_THIRD_PARTY_DETAILS_HIST', 'EXT$TTMN_AML');
	DBMS_DATAPUMP.DATA_FILTER (v_dp_job, 'SUBQUERY', 'WHERE to_date(V_MIS_DATE,''YYYYMMDD'') >= add_months(trunc(sysdate,''MM''),0)', 'CASH_TXN_ATM_DETAILS_HIST', 'EXT$TTMN_AML');
	DBMS_DATAPUMP.DATA_FILTER (v_dp_job, 'SUBQUERY', 'WHERE to_date(V_MIS_DATE,''YYYYMMDD'') >= add_months(trunc(sysdate,''MM''),0)', 'CASH_TXN_BRANCH_DETAILS_HIST', 'EXT$TTMN_AML');
	DBMS_DATAPUMP.DATA_FILTER (v_dp_job, 'SUBQUERY', 'WHERE to_date(V_MIS_DATE,''YYYYMMDD'') >= add_months(trunc(sysdate,''MM''),0)', 'CASH_TXN_CARD_DETAILS_HIST', 'EXT$TTMN_AML');
	DBMS_DATAPUMP.DATA_FILTER (v_dp_job, 'SUBQUERY', 'WHERE to_date(V_MIS_DATE,''YYYYMMDD'') >= add_months(trunc(sysdate,''MM''),0)', 'CASH_TXN_PICKUP_DETAILS_HIST', 'EXT$TTMN_AML');
	DBMS_DATAPUMP.DATA_FILTER (v_dp_job, 'SUBQUERY', 'WHERE to_date(V_MIS_DATE,''YYYYMMDD'') >= add_months(trunc(sysdate,''MM''),0)', 'CASH_TXN_POS_DETAILS_HIST', 'EXT$TTMN_AML');
	DBMS_DATAPUMP.DATA_FILTER (v_dp_job, 'SUBQUERY', 'WHERE to_date(V_MIS_DATE,''YYYYMMDD'') >= add_months(trunc(sysdate,''MM''),0)', 'EFT_TXN_INST_DETAILS_HIST', 'EXT$TTMN_AML');
	DBMS_DATAPUMP.DATA_FILTER (v_dp_job, 'SUBQUERY', 'WHERE to_date(V_MIS_DATE,''YYYYMMDD'') >= add_months(trunc(sysdate,''MM''),0)', 'EFT_TXN_TRANFER_DETAILS_HIST', 'EXT$TTMN_AML');
	DBMS_DATAPUMP.DATA_FILTER (v_dp_job, 'SUBQUERY', 'WHERE to_date(V_MIS_DATE,''YYYYMMDD'') >= add_months(trunc(sysdate,''MM''),0)', 'MI_TXN_CHECK_DETAILS_HIST', 'EXT$TTMN_AML');
	DBMS_DATAPUMP.DATA_FILTER (v_dp_job, 'SUBQUERY', 'WHERE to_date(V_MIS_DATE,''YYYYMMDD'') >= add_months(trunc(sysdate,''MM''),0)', 'MI_TXN_INST_DETAILS_HIST', 'EXT$TTMN_AML');	
